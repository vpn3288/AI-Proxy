...
_restore_prev_port_traps(){
  if [[ -n "${_prev_err_trap:-}" ]]; then eval "$_prev_err_trap"; else trap - ERR; fi
...
install_xray_binary(){
  info "下载 Xray-core ..."
  local ver
  ver=$(curl -fsSLI --connect-timeout 10 -o /dev/null -w '%{url_effective}'     "https://github.com/XTLS/Xray-core/releases/latest" 2>/dev/null | awk -F/ 'NF{print $NF}' | tail -1)
  ...
  if ! wget -q --timeout=10 --tries=2 -O "${tmp_dir}/sha256sums.txt" \
      "https://github.com/XTLS/Xray-core/releases/download/${ver}/sha256sums.txt" 2>/dev/null; then
    die "Xray sha256sums.txt 下载失败，拒绝安装未验证二进制"
  fi
  [[ -f "${tmp_dir}/sha256sums.txt" ]] || die "sha256sums.txt 下载后缺失"
  grep -qF "$zip_name" "${tmp_dir}/sha256sums.txt" || die "sha256sums.txt 中未找到 ${zip_name}，拒绝安装未验证二进制"
...
setup_fallback_decoy(){
  ...
  for _check_port in 45231 45232; do
    ...
    while IFS= read -r _proc; do
      [[ -z "$_proc" ]] && continue
      [[ "$_proc" == nginx ]] || _bad=1
    done < <(ps -o comm= -p ${_pid_list} 2>/dev/null | sed '/^$/d' || true)
...
FDEOF
  ...
  if systemctl is-active --quiet nginx 2>/dev/null; then
    systemctl reload nginx \
      || die "nginx reload 失败（配置已验证通过但reload异常），请检查: journalctl -u nginx"
  else
...
issue_certificate(){
  ...
  if [[ ! -f "${ACME_HOME}/acme.sh" ]]; then
    ...
    env HOME="${_acme_tmp_home}" ACME_HOME="${ACME_HOME}" curl -fsSL https://get.acme.sh       | env HOME="${_acme_tmp_home}" ACME_HOME="${ACME_HOME}" sh -s           --home "${ACME_HOME}"           email="admin@${domain}"           --nocron           --log "/var/log/acme-xray-landing-renew.log"       || { rm -rf "${_acme_tmp_home}" 2>/dev/null || true; die "acme.sh 安装失败"; }
